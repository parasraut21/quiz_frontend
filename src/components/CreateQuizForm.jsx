...
const CreateQuizForm = () => {
  ...
  const handleCorrectOptionChange = (index, optionIndex) => {
    ...
    if (updatedQuestions[index].multiCorrect) {
      ...
      if (correctOptions.includes(optionIndex)) {
        updatedQuestions[index].correctOption = correctOptions.filter(
          (opt) => opt !== optionIndex
        );
      } else {
        updatedQuestions[index].correctOption = [
          ...correctOptions,
          optionIndex,
        ];
      }
  ...
  const handleMultiCorrectChange = (index, checked) => {
    ...
    if (!checked) {
      updatedQuestions[index].correctOption =
        updatedQuestions[index].correctOption.length > 0
  ...
  const validateForm = () => {
    ...
    if (!quizCreator.trim()) {
      errors.push("Quiz Creator name is required");
    }

    if (!topic.trim()) {
      errors.push("Quiz Topic is required");
    }
    ...
    questions.forEach((question, index) => {
      ...
      if (question.options.some((option) => !option.trim())) {
        errors.push(`All options for Question ${index + 1} must be filled`);
      }

      if (question.correctOption.length === 0) {
        errors.push(
          `Correct option(s) for Question ${index + 1} must be selected`
        );
      }
    });
      ...
      <form className="space-y-8">
                ...
                >
                      ...
                      {question.options.map((_, optIndex) => (
                        ...
                        >
                          <input
                            ...
                            onChange={() =>
                              handleCorrectOptionChange(index, optIndex)
                            }
                  ...
                  <div className="flex items-center bg-gray-700 p-3 rounded-lg">
                    <input
                      ...
                      onChange={(e) =>
                        handleMultiCorrectChange(index, e.target.checked)
                      }
                      className="form-checkbox h-5 w-5 text-purple-600"
                    />
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center bg-gray-700 rounded-lg">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            handleDifficultyChange(index, e.target.value)
                          }
                          required
                        >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center bg-gray-700 rounded-lg">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleTimeLimitChange(index, e.target.value)
                          }
                          min="10"
                          required
                        />
        ...
        <div className="flex justify-between mt-6">
          ...
          <motion.button
            ...
            disabled={
              currentQuestionIndex === questions.length - 1 || !isFormValid()
            }
          >
            Next
            <svg
            ...
            >
              ...
              <div className="space-y-4 mb-6">
                <p className="text-gray-300">
                  <span className="font-bold">Quiz Creator:</span> {quizCreator}
                </p>
                <p className="text-gray-300">
                      ...
                      {question.options.map((option, optIndex) => (
                        <p
                          ...
                          className={`p-2 rounded flex items-center ${
                            question.correctOption.includes(optIndex)
